...
export default ({route}) => {
    ...
    return (
        <View style = {{alignItems:'center', flex:1, width:width}}>
            <View style = {{flexDirection:'row', justifyContent:'center', marginTop:height*.05, marginBottom: height*.05}}>
                    ...
                    {adminContext.state.comments.map((item,index) => {
                        return(
                            <View key = {index} style = {{alignItems:'center'}}>
                                ...
                                <TouchableOpacity onPress = {() => adminContext.deleteComment(item.id)}>
