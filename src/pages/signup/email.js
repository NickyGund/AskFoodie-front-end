...
export default ({ navigation }) => {
  ...
  const next = async () => {
    if (!checkEmail()) return setFormatted(false);
    setClicked(true);
    try {
      const res = await authContext.checkEmail(email);
      if (res) {
        setTaken(true);
      } else {
  ...
  return (
    <SafeAreaView style={{flex:1}}>
      ...
      <View style={{paddingHorizontal:'10%'}}>
        <Text style={{ fontSize: width * 0.06, marginTop:'10%' }}>Create Email</Text>
        <TextInput
          placeholder="Email"
        ...
        />
        {!formatted && <Text style={[s.mt5, { color: 'red' }]}>Please Enter a Valid Email</Text>}
        {taken && <Text style={[s.mt5, { color: 'red' }]}>This email already exists</Text>}
        <TouchableOpacity
          disabled={email === ''}
          onPress={next}
          style={{ opacity: email === '' ? 0.5 : .8, marginTop:'5%' }}
        >
          <View
