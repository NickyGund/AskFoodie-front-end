...
const AuthProvider = props => {
  ...
  const signUp = async () => {
    try {
      const res = await axios.post('http://192.168.1.201:19000/api/register', {
        email:email,
        firstName:name,
        userName:userName,
        birthdate,
  ...
  const signIn = async () => {
    try{
      ...
      const res = await axios.post('http://192.168.1.201:19000/api/login', {
        email:email,
        password:password
      });
  ...
  const checkUserName = async value => {
    try {
      const res = await axios.get(`http://192.168.1.201:19000/api/check_username/${value}`);
  ...
  const checkEmail = async value => {
    try {
      const res = await axios.get(`http://192.168.1.201:19000/api/check_email/${value}`);
